...
(() => {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    console.log('[Core X] bootstrap start');
    try { initStarfield(); } catch (e) { console.error('[Core X] initStarfield 실패:', e); }
    try { Board.init(document.getElementById('board-canvas'), handleBoardClick); }
    catch (e) { console.error('[Core X] Board.init 실패:', e); }
    try { wireEvents(); } catch (e) { console.error('[Core X] wireEvents 실패:', e); }
    try { show('title'); } catch (e) { console.error('[Core X] show(title) 실패:', e); }
    // 스프라이트 로드 완료 후 화면 재렌더 (안전하게)
    try {
      if (window.Sprites && typeof Sprites.onLoaded === 'function') {
        Sprites.onLoaded(() => {
          try {
            const cs = document.getElementById('screen-char-select');
            if (cs && cs.classList.contains('active')) renderCharGrid();
            if (Game.State.players && Game.State.players.length > 0) renderHUD();
          } catch (e) { console.warn('[Core X] sprite-loaded callback:', e); }
        });
      }
    } catch (e) { console.warn('[Core X] Sprites.onLoaded 등록 실패:', e); }
    console.log('[Core X] bootstrap done');
  });
})();
